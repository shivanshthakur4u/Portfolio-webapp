...
const CustomLink = ({href,title,className=""})=>{
    ...
    return(
        <Link href={href} className={`${className} relative group`}>
          {title}
          <span className={`h-[1px] inlline-block  bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 ${router.asPath===href ? 'w-full':'w-0'}`}>&nbsp;</span>
        </Link>
    )
}
...
const Navbar = () => {
 ...
 <motion.a  href="/" target='_blank' whileHover={{y:-2}}   whileTap={{scale:0.9}} className='w-6 mx-3'><TwitterIcon/></motion.a>
    ...
    <motion.a href="/" target='_blank' whileHover={{y:-2}} className='w-6 mx-3'><PinterestIcon/></motion.a>
